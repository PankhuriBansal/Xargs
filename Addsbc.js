...
sendData(ev){
  ...
  const sbcname=this.state.sbcname
  ...
  const data={
    platform,sbc,sbcname,Id,platform1
     
      }
      Axios.get('http://localhost:8080/examples/xargs/addsbc.jsp',{params:data}).then(response=>{console.log(response);
      this.setState({
...
<TextField variant="outlined" margin="normal" required fullWidth id="email" name="sbcname" value={this.state.sbcname} autoFocus onChange={this.fun.bind(this)} label="SBC Name"  autoFocus /> 
                  
<TextField variant="outlined" margin="normal" required fullWidth  label="SBC Unique ID" type="text" id="password" autoComplete="current-password" name="Id" value={this.state.Id} autoFocus onChange={this.fun.bind(this)}  />
